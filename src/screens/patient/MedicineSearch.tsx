// Medicine Search - Enhanced patient medicine search with 50k+ database
...
const MedicineSearch: React.FC = () => {
  ...
  const loadSearchHistory = async () => {
    try {
      ...
      const history = ['paracetamol', 'ibuprofen', 'amoxicillin', 'metformin', 'omeprazole']
  ...
  const handleMedicinePress = async (medicine: Medicine) => {
    try {
      ...
      if (!searchHistory.includes(medicine.genericName)) {
        const newHistory = [medicine.genericName, ...searchHistory.slice(0, 9)]
